...
Terrain.prototype.draw = function(ctx, width, height, color) {
    var self = this;
    var waterVal = this.size * 0.3;

    //Okay... First, we create nested loops that drew rectangles from the 'back' (y = 0) to the 'front' (y = this.size) of our map. 
    ...
    for (var y = 0; y < this.size; y++) {
        ...
        var style = brightness(x, y, this.get(x + 1, y) - val, color);
    ...
    //brightness controll
    function brightness(x, y, slope, def_color) {
        if (y === self.max || x === self.max) return def_color;
        var b = ~~(slope * 50) + 128;
...
//Biomes Data
//Setting up Biomes
var Biome = function(roughness, color){
    this.roughness = roughness;
    this.color = color;
}
//Creating 4 main Biomes
Beach = new Biome(.1, "#F0E68C");
Hills = new Biome(.8, "#A9A9A9");
Grasslnd = new Biome(.5, "#006400");
Mountains = new Biome(1, "#A9A9A9");

var biomes = [Beach, Mountains, Grasslnd, Hills]
...
//Setting data for canvas & terrain info
...
//Generating
terrain.generate(biome.roughness);
terrain.draw(ctx, width, height, biome.color);